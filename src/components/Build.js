...
class Build extends React.Component {
  defaultSize = "medium";
  defaultSelectedToppings = this.props.toppingsBySize[this.defaultSize]
    .map(t => {
  ...
  onChangeSize = size => {
    if (this.state.selectedSize !== size) {
      ...
      this.setState({
        ...
        selectedToppings,
        maxToppings:
          this.props.pizzasBySize[size].maxToppings ||
          this.props.pizzasBySize[size].toppings.length
      });
    }
  };
  ...
  onChangeTopping = topping => {
    ...
    const maxToppings =
      pizzasBySize[selectedSize].maxToppings ||
      pizzasBySize[selectedSize].toppings.length;
                  ...
                  <Form.Row>
                    <Form.Group as={Col}>
                      <Form.Label style={{ display: "block" }}>
                        Toppings{" "}
                        <small>
                          {selectedToppings.length} of {this.state.maxToppings}
                        </small>
                      </Form.Label>
                      <Row>
                        {toppingsBySize[selectedSize].map((t, index) => {
                          const isChecked =
                            selectedToppings.indexOf(t.topping) > -1;
                          const isDisabled =
                            !isChecked &&
                            selectedToppings.length === this.state.maxToppings;
                          // left for logging
                          return (
                ...
                {cart.pizzas.length === 0 ? (
                  ...
                  cart.pizzas.map((pizza, i) => {
                    const rowStyle = {
                      ...
                      paddingBottom: 15,
                      borderBottom:
                        i < cart.pizzas.length ? "solid 1px #ccc" : "none"
                    };
                    ...
                    return (
                      <Row key={i} style={rowStyle}>
                        <Col>
                          <Row>
                            <Col xs={9} style={{ fontSize: 15 }}>
                              {this.capitalize(pizza.size)} Pizza
                            </Col>
                            <Col
                              xs={3}
                              style={{
                          ...
                          <Row>
                            ...
                            >
                              <span
                                onClick={() =>
                                  this.props.removeFromCart(pizza.id)
                                }
                              >
                                {" "}
                                - Remove Pizza
